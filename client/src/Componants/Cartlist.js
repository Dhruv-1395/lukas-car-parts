...
const Cartlist = () => {
  ...
  const handleRemove = (id) => {
    ...
    axios.delete('http://localhost:5000/cart/' + id)
      .then(res => {
        if (res) { toast.success('Item removed!') }
                  ...
                  <tbody>
                    {
                      items.map((items) => {
                        return (
                          <tr key={items.id}>
                            <td> <img src={require('../assets/' + items.img)} width={70} alt="item" /></td>
                            <td>{items.pname}</td>
                            <td>{items.color}</td>
                            <td>{items.size}</td>
                            <td>₹{items.price}</td>
                            ...
                            <td><button className='btn btn-danger' onClick={() => handleRemove(items.id)}>Remove</button></td>
                          </tr>
                        );
                      })
                    }
          ...
          <div className="col-5">
            <table className='table' >
              <tbody>
                ...
                <tr>
                  <td><FaTruckArrowRight />  Dilivery Charges :</td>
                  <td>₹{(subtotal * 2) / 100}</td>
                </tr>
                <tr>
                  <td style={{ fontWeight: 600, color: "#eeb644" }}>Grand-Total :</td>
                  <td style={{ fontWeight: 600, color: "#eeb644" }}>₹{subtotal + ((subtotal * 18) / 100) + ((subtotal * 2) / 100)}</td>
                </tr>
              </tbody>
            </table>
